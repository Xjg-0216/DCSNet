...
import cv2
from basicsr.data.transforms import MixUp_AUG
...
def plot_feature(feature):
    output1 = feature.cpu().numpy()
    output1 = np.squeeze(output1)
    output1 = (((output1 - np.min(output1)) / (np.max(output1) - np.min(output1))) * 255)
    output1 = output1.astype(np.uint8)
    output1 = output1.transpose((1, 2, 0))
    # plt.imshow(att, cmap='magma')
    # output1 = cv2.applyColorMap(output1, cv2.COLORMAP_JET)
    return output1
...
class ImageRestorationModel(BaseModel):
    ...
    def feed_mixup_data(self, data):
        img_gt, img_lq = MixUp_AUG().aug(data['gt'], data['lq'])
        self.lq = img_lq.to(self.device)
        if 'gt' in data:
            self.gt = img_gt.to(self.device)
    ...
    def print_average_psnr(self, average_psnr, current_iter, tb_logger):
        log_str = 'current_iter: {}, {}: {:.4f}'.format(current_iter, 'average_psnr', average_psnr)
        logger = get_root_logger()
        logger.info(log_str)
        if tb_logger:
            tb_logger.add_scalar(f'metrics/average_psnr', average_psnr, current_iter )
